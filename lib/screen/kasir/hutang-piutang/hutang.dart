...
class HutangPage extends StatefulWidget {
  @override
  createState() => HutangPageState();
}

class HutangPageState extends State<HutangPage> {
  RefreshController _refreshController =
      RefreshController(initialRefresh: false);
  List<HutangModel> listHutangs = [];
  List<HutangModel> tmpHutangs = [];
  ...
  void getData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status == 200) {
          datas.forEach((data) {
            listHutangs.add(HutangModel.fromJson(data));
            tmpHutangs.add(HutangModel.fromJson(data));
          });
                  ...
                  : listHutangs.length == 0
                      ...
                      : SmartRefresher(
                          ...
                          child: ListView.separated(
                              ...
                              itemCount: listHutangs.length,
                              separatorBuilder: (_, i) => SizedBox(height: 10),
                              itemBuilder: (ctx, i) {
                                var _hutang = listHutangs[i];
                                return _buildLitle(i, _hutang);
                              }),
                        ),
            ),
          ],
  ...
  Widget _buildLitle(int index, HutangModel hutang) {
    return InkWell(
      onTap: () async {
        dynamic response = await Navigator.of(context).push(MaterialPageRoute(
          builder: (context) => MutasiHutang(hutang),
        ));
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          subtitle: hutang.jatuhTempo != ''
              ? Text(
                  formatDate(hutang.jatuhTempo, 'd MMMM yyyy'),
