...
import '../../../screen/home/home4/home4.dart';
import 'index.dart';
...
class _MyQrisPageState extends State<MyQrisPage> {
  ...
  Future<Map<String, dynamic>> _getQr() async {
    try {
      ...
      final url = Uri.parse('https://santren-app.findig.id/api/v1/qris/generate');
      ...
      if (data != null && data['image'] != null && data['image'].toString().trim().isNotEmpty) {
        print('[DEBUG] Berhasil dapat image URL: ${data['image']}');
        return {
          'status': 200,
          'image': data['image'],
        };
      } else if (data != null && data['status'] == 200 && (data['image'] == null || data['image'].toString().trim().isEmpty)) {
...
Widget _waitingBankVerificationPage() {
  ...
  child: Container(
    decoration: BoxDecoration(
      ...
      borderRadius: BorderRadius.circular(16), // Jika mau border radius
    ),
    child: ClipRRect(
      borderRadius: BorderRadius.circular(16), // Samakan dengan border di atas
      child: Image.asset(
  ...
  Widget _getQrisWidget(Map<String, dynamic> result) {
    ...
    else if (status == 200 && (result['image'] == null || result['image'].toString().trim().isEmpty)) {
      return _waitingBankVerificationPage();
    }
     
   else if (status == 200 && result['image'] != null && result['image'].toString().trim().isNotEmpty) {
