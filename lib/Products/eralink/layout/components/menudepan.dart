...
import 'package:mobile/Products/eralink/layout/components/moremenu.dart';
...
class _MenuDepanState extends State<MenuDepan> {
  ...
  getMenu() async {
    try {
      List<dynamic> datas = await api.get('/menu/1', cache: false);
      ...
      if (listMenu.length > (widget.grid * totalBaris)) {
        ...
        MenuModel buttonMore = MenuModel(
            jenis: 99,
            icon:
                'https://dokumen.payuni.co.id/logo/centralbayar/new/icon/centrallainya.png',
            name: 'Lainnya',
            type: 99);
