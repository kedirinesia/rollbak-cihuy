...
class _SelectKotaPageState extends State<SelectKotaPage> {
  List<MarketplaceKota> master = [];
  List<MarketplaceKota> items = [];
  bool isLoading = true;
  ...
  void getItems() async {
    ...
    if (response.statusCode == 200) {
      ...
      datas.forEach((data) => master.add(MarketplaceKota.fromJson(data)));
      items = master;
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Pilih Kota')),
      body: Container(
