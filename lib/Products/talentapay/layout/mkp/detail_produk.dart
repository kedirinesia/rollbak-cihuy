...
class ProductDetailMarketplace extends StatefulWidget {
  ...
  ProductDetailMarketplace(this.id, this.title, this.categoryId);
...
class _ProductDetailMarketplaceState extends State<ProductDetailMarketplace> {
  ...
  List<ProdukMarket> relatedProducts = [];
  ...
  Future<void> getProduct() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          product = ProdukDetailMarket.fromJson(data);
          loading = false;
        });
  ...
  Future<void> getRelatedProducts(String categoryId) async {
    try {
      final response = await http.get(
        Uri.parse('$apiUrl/market/${widget.categoryId}/products?limit=10'),
        headers: {'Authorization': bloc.token.valueWrapper?.value},
      );
